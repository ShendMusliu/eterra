...
import { env } from "$amplify/env/admin-start-user-role-import";
...
export const handler: Handler = async (event: AppSyncEvent) => {
  ...
  const sanitizedName =
    typeof originalFileName === "string" && originalFileName.trim().length > 0
      ...
      : `user-role-import-${timestamp}.csv`;
  const csvKey = `${prefix}/${jobId}/source.csv`;

  await uploadCsvToS3(csvKey, csv);

  const jobInput: Parameters<typeof dataClient.models.UserRoleImportJob.create>[0] = {
    id: jobId,
    status: "QUEUED",
    csvS3Key: csvKey,
  ...
  const createResult = await dataClient.models.UserRoleImportJob.create(jobInput);
