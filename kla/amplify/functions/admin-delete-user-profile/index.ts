...
import { env } from "$amplify/env/admin-delete-user-profile";
...
export const handler: Handler = async (event: AppSyncEvent) => {
  ...
  const existing = await client.models.UserProfile.get({ pk, sk: PROFILE_SK });
  if (existing.errors?.length) {
    throw new Error(existing.errors.map((error) => error.message ?? "Unknown error").join(", "));
  }
  ...
  const result = await client.models.UserProfile.delete({ pk, sk: PROFILE_SK });
